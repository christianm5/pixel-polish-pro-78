...
const Donate = () => {
  const { get } = useSiteContent("donate");
  ...
  const handleDonate = async () => {
    if (!finalAmount || finalAmount <= 0) { toast.error("Veuillez choisir un montant."); return; }
    ...
    <Layout>
      <section className="py-20 lg:py-28 bg-hero-gradient text-primary-foreground">
        <div className="container mx-auto px-4 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-2xl mx-auto space-y-4">
            ...
            <h1 className="font-display text-4xl lg:text-5xl font-bold">{get("hero", "title", "Faire un Don")}</h1>
            <p className="text-primary-foreground/70 font-body text-lg">{get("hero", "description", "Votre générosité transforme des vies.")}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28">
        <div className="container mx-auto px-4 lg:px-8 max-w-2xl">
          <SectionHeader subtitle={get("form", "subtitle", "Contribuer")} title={get("form", "title", "Choisissez un Montant")} />
          <motion.div {...fadeInUp} className="space-y-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {amounts.map((amount) => (
                <button key={amount} onClick={() => { setSelectedAmount(amount); setCustomAmount(""); }}
                  className={`p-4 rounded-lg border-2 font-display text-xl font-bold transition-all duration-200 hover:shadow-elevated ${selectedAmount === amount ? "border-primary bg-primary/10 text-primary" : "border-border bg-card text-card-foreground hover:border-primary"}`}>
                  ${amount}
                </button>
              ))}
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" placeholder="Montant personnalisé" value={customAmount}
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={method.id} onClick={() => setPaymentMethod(method.id)}
                    className={`flex items-center gap-3 p-4 rounded-lg border cursor-pointer transition-colors ${paymentMethod === method.id ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary"}`}>
